...
import logging
import numpy as np
from config.vars import ticker_symbols, initial_capital, upper_limits, \
    lower_limits
...
def setup_logging(symbol):
    ...
    log_filename = os.path.join(log_dir,
                                f'{symbol.replace(".", "_")}_optimal_parameter_{results_date_str}.log')
...
class TradeModel:
    ...
    def buy_stock(self, price, quantity):
        ...
        if quantity > 0:
            ...
            self.average_price = ((self.average_price * (
                        self.holding_quantity - quantity)) + (
                                              price * quantity)) / self.holding_quantity
            logging.info(
    ...
    def sell_stock(self, price, quantity):
        ...
        if quantity > 0:
            ...
            if self.holding_quantity == 0:
                self.average_price = 0
            logging.info(
                f"Sold {quantity} shares at {price} each. New capital: {self.capital}, Holding: {self.holding_quantity}, Average purchase price: {self.average_price}")
...
class TradeController:
    ...
    def trading_logic(self, current_price, upper_limit, lower_limit):
        ...
        short_term_ma, long_term_ma = short_term_ma[-min_length:], long_term_ma[
                                                                   -min_length:]

        self.logger.info(
            f"Short-term MA: {short_term_ma[-1]}, Long-term MA: {long_term_ma[-1]}")
        self.logger.info(
        ...
        self.logger.info(
            f"Action: {action}, Quantity: {quantity}, Price: {current_price}")
        return action, quantity
...
def optimize_parameters(df, upper_limit, lower_limit, ticker_symbol):
    ...
    final_value = trade_controller.model.capital + trade_controller.model.holding_quantity * \
                  df.iloc[-1]['Close']
...
def main():
    for ticker_symbol in ticker_symbols:
        ...
        csv_filename = os.path.join("stockdata",
                                    f'{ticker_symbol.replace(".", "_")}_one_month_daily_stock_data_{stockdata_date_str}.csv')
        df = load_stock_data(csv_filename)

        param_combinations = [(ul, ll) for ul in upper_limits for ll in
                              lower_limits]
        ...
        for upper_limit, lower_limit in param_combinations:
            final_value, profit_loss = optimize_parameters(df, upper_limit,
                                                           lower_limit,
                                                           ticker_symbol)
        ...
        results_df = pd.DataFrame(results,
                                  columns=['upper_limit', 'lower_limit',
                                           'final_value', 'profit_loss'])
        results_filename = os.path.join(log_dir,
                                        f'{ticker_symbol.replace(".", "_")}_optimal_parameters_results_{datetime.datetime.now().strftime("%Y%m%d%H%M")}.csv')
